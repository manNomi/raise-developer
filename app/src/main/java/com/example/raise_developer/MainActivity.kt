...
class MainActivity : AppCompatActivity() {
    ...
    val handler =
        Handler(Looper.getMainLooper()) { // 메시지를 받을 때 마다 plusAnnualMoneyToPersonalMoney() 이 함수 실행
...
//        characterView.layoutParams = param
    ...
    inner class CalculateAnnualMoney :
        Runnable { // 쓰레드 클래스 isThreadStop이 false일 때 돌아가며, 5초마다 handler에 메세지를 보내줌
        override fun run() {
            while (!isThreadStop) {
                Thread.sleep(5000)
